...
import Control.Monad.State (class MonadState, StateT, evalStateT, gets, modify_, state)
...
type Pos = { position :: Position, sensitive :: Boolean }

newtype Positioned m a = Positioned (StateT Pos m a)
...
runIndentT (Positioned f) = evalStateT f { position: initialPos, sensitive: false }

getIndentLevel :: forall m. Monad m => IndentParserT m Position
getIndentLevel = do
  g <- lift (Positioned (gets (\pos -> pos.position)))
  pure g
...
withPos x = do
  ...
  p <- position
  sensitize
  r <- putIndentLevel p *> x
  putIndentLevel a *> pure r
...
sensitize :: forall m. Monad m => IndentParserT m Unit
sensitize = lift $ Positioned (modify_ (\pos -> pos { sensitive = true }))

desensitize :: forall m. Monad m => IndentParserT m Unit
desensitize = lift $ Positioned (modify_ (\pos -> pos { sensitive = false }))

isSensitive :: forall m. Monad m => IndentParserT m Boolean
isSensitive = lift (Positioned (gets (\pos -> pos.sensitive)))

whenSensitive :: forall m. Monad m => IndentParserT m Unit -> IndentParserT m Unit
whenSensitive f = do
  s <- isSensitive
  when s f
...
indented = whenSensitive do
  Position p <- position
  Position s <- getIndentLevel
  if p.column <= s.column then fail "not indented"
  else putIndentLevel $ Position { index: 0, line: p.line, column: s.column }
